...
def _require_viewer_role():
    ...
    if not (_VIEWER_ROLES & user_roles):
        frappe.throw(
            _("You do not have permission to access Activity Tracker dashboards. "
...
def get_user_dashboard(user: str = None, period: str = "today") -> dict:
    ...
    today_str = today()
    if period == "week":
        from_date = str(getdate(today_str) - __import__("datetime").timedelta(days=7))
    elif period == "month":
        from_date = str(getdate(today_str) - __import__("datetime").timedelta(days=30))
    else:
        from_date = today_str
